from django.shortcuts import render, redirect
from .models import Article
from django.http import HttpResponse
from django.contrib.auth.decorators import login_required
from . import forms

# Create your views here.
def articleList(request):
    articles = Article.objects.all().order_by("date") # order by date attribute
    return (render(request, "articles/articleList.html", {"articles": articles}))

def articleDetails(request, slug):
    article = Article.objects.get(slug=slug)
    return (render(request, "articles/articleDetail.html", {"article": article}))

@login_required(login_url="/accounts/login")
def articleCreate(request):
    if request.method == "POST":
        form = forms.CreateArticle(request.POST, request.FILES) # validate form after "create" btn is submitted
        if form.is_valid():
            instance = form.save(commit=False) # don't commit to DB yet
            instance.author = request.user
            instance.save()
            return(redirect("articles:list"))
    else:
        form = forms.CreateArticle()
    return (render(request, "articles/articleCreate.html", {"form": form}))